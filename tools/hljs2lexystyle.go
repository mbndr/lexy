...
func main() {
	...
	for _, info := range fl {
		...
		styleList = append(styleList, toStyleVarName(n))
	...
	// generate style list
	f, err := os.Create(filepath.Join(*outputDir, "stylelist.go"))
	if err != nil {
		log.Fatal(err)
	}
	defer f.Close()

	err = generateStyleList(f, styleList)
	if err != nil {
		log.Fatal(err)
	}

	// run gofmt
	cmd := exec.Command("go", "fmt", "./" + *outputDir)
...
// generateStyleList writes a list with all styles to access them easily
func generateStyleList(w io.Writer, l []string) error {
	t, err := template.New("").Parse(styleListTemplate)
	if err != nil {
		return err
	}
...
var fileTemplate = `package style
...
var styleListTemplate = `package style
// Code generated by mbndr/lexy
// DO NOT EDIT!
// @generated
